...
app.post("/upload", authMiddleware, (req, res) => {
    ...
    // 1. Capturamos el tamaño total del archivo desde las cabeceras
    const fileSize = parseInt(req.headers['content-length'] || "0");

    bb.on('file', (name, file, info) => {
        if (name === "thumbnail") {
            const c = []; 
            file.on('data', d => c.push(d));
            file.on('end', () => thP = uploadThumbnailBuffer(Buffer.concat(c)).catch(()=>null));
        } else if (name === "file") {
            fName = info.filename; 
            mime = info.mimeType;
            // 2. Pasamos el fileSize a la función de Telegram para calcular las partes
    ...
    bb.on('close', async () => {
        if (!vidP) return res.status(400).json({ error: "No se subió ningún archivo" });
        try {
            const [vid, th] = await Promise.all([vidP, thP]);
            
            // Procesar ID de miniatura
            let tId = th ? (th.message_id || th) : null;
            if (typeof tId === 'object') tId = JSON.stringify(tId);

            // 3. Guardar en Supabase INCLUYENDO EL 'SIZE'
            const { error } = await supabase.from('files').insert([{
                user_id: req.user.id, 
                name: fName, 
                mime: mime,
                size: fileSize, // <--- Dato clave guardado
                thumbnail_id: tId ? String(tId) : null,
                telegram_id: String(vid.telegram_id), 
                message_id: String(vid.message_id)
            }]);
...
app.get("/files", authMiddleware, async (req, res) => {
    // Obtenemos archivos del usuario ordenados por fecha
    ...
    if(error) return res.status(500).json({ error: "Error al obtener archivos" });
    res.json(data);
});

app.get("/file-url/:file_id", authMiddleware, async (req, res) => {
    // Obtener URL temporal de la miniatura
...
app.get("/stream/:message_id", authMiddleware, async (req, res) => {
    try {
        // 1. Buscamos info del archivo en BD para saber MimeType y Nombre
        ...
        // 2. Capturamos el Rango que pide el navegador (importante para adelantar video)
        const range = req.headers.range;

        if (f) {
            // Seteamos cabeceras básicas
            res.setHeader('Content-Type', f.mime);
            res.setHeader('Content-Disposition', `inline; filename="${f.name}"`);
        }

        // 3. Iniciamos el streaming inteligente (multibot)
        await streamFile(req.params.message_id, res, range);

    } catch (error) {
        console.error("❌ Error Stream Route:", error);
